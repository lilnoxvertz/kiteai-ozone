...
async function main() {
    ...
    try {
        ...
        wallets.forEach((address, index) => {
            authDataArr.push({
                eoa: authData[index].eoa,
                authToken: authData[index].authToken,
                cookie: authData[index].cookie
            })
        })

        for (let j = 0; j < authData.length; j++) {
            const eoa = authDataArr[j].eoa
            const authToken = authDataArr[j].authToken
            const cookie = authDataArr[j].cookie
            const proxy = await Proxy.getProxy(proxys, j)
            dailyAgentTask.push(() => Workers.dailyAgent(eoa, authToken, cookie, proxy))
        }

        skibidi.processing("[COMPLETING DAILY INTERACTION]")
        await Workers.limitTasks(dailyAgentTask, maxWorker)
    } catch (error) {
